...
$$%
%
По физическому смыслу $m$ -- это проекция вектора $\vec{j}$ на ось $Oz$, $\lambda(j)$ -- квадрат длины вектора углового момента. Попробуем разобраться, какие значения могут принимать $\lambda(j)$ и $m$, пользуясь только коммутационными соотношениями.

Прежде всего заметим, что так как в любом состоянии
$$
...
$$%
%
Очевидно, что $m_{max} = - m_{min}$. Пусть $m_{max} \equiv j$ (верхняя граница изменения проекции вектора $\vec{j}$ на ось $Oz$ лимитирована его модулем), тогда $m_{min} = -j$.
...
\end{equation}%
%
При $s=1/2$ проекция спина на выделенное направление, например, на ось $Oz$, может принимать $2s + 1 = 2$ значения: $m_s = \pm 1/2$. Базисные векторы этого представления $\chi_{\frac{1}{2}, m_s} \equiv \ket{\frac{1}{2}, m_s}$ имеют вид
...
\end{equation}%
%
Из условия нормировки $\bk{\chi}{\chi} = 1$ следует, что 
$$
\abs{a_{+}}^2 + \abs{a_{-}}^2 = 1
$$%
%
...
%people
\footnotetext{Вольфганг Эрнст Паули (Wolfgang Ernst Pauli, 1900-1958)}
...
В классической механике {\em момент количества движения частицы} (или {\em орбитальный момент}) определяется как векторное произведение радиус-вектора $\vr$ частицы и её импульса $\vp$ (см.~\llref{9}{1})
$$
\vec{L} = \vr \times \vp
$$%
%
...
%people
\footnotetext{Адриен Мари Лежандр (Adrien-Marie Legendre, 1752-1833)}
