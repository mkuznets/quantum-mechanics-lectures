...
$\op{G} g_n \ket{n}$, где $\ket{n} \equiv \ket{\psi_n},~n \in \mathbb{Z}$
...
$F_{nn'} = \bfk{n}{\op{F}}{n'}$ --- матричное представление $\op{F}$ в базисе состояний $\{\ket{n}\}$ собственных векторов $\op{G}$ ($\op{G}=\op{H}$, $E$-представление)
...
$$
\bfk{n}{\op{A}\op{B}}{n''} = \sum_{n'} \bfk{n}{\op{A}}{n'} \bfk{n'}{\op{B}}{n''} = \sum_{n'} A_{n'} B_{n'n''} \text{(произведение матриц)}
$$
...
Пусть $\op{F}$ --- оператор физической величины в $\lambda$-представлении, а $\op{F'}$ --- в $\mu$-представлении.
...
\item Унитарные преобразования сохраняют коммутационные соотношения: если $[\op{F}, \op{G}]=\op{K}$, то $[\op{F'}, \op{G'}]=\op{K'}$
...
$$
\begin{gathered}
\left\{\ket{n^{(i)}}\right\} = \left\{\ket{n'^{(i)}}\right\} \\
...
$\ket{n'^{(1)}}, \ket{n'^{(2)}}... \ket{n'^{(1k)}}$ --- собственные векторы
...
Объединяя \eqref{eq:6_3_17} и \eqref{eq:6_3_18}, получим стационарное уравнение (интегральное) Шрёдингера в импульсном представлении:
\begin{equation}
\label{eq:6_3_17}
	\boxed{\frac{p^2}{2m} \psi(\vp) + \int W(\vp - \vsp) \psi(\vsp) \, d\vp  = E\psi(\vp)}
\end{equation}

Из упражнения \No1 2-го задания:
$$
\begin{gathered}
\op{\vr} = i\hbar \pd{}{\vp} \\
\op{\vp} = -i\hbar \pd{}{\vr}
