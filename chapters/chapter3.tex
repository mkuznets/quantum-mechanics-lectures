...
\section{Состояние и волновая функция. Принцип суперпозиции состойний. Дираковская формулировка квантовой механики. Вектор состояния.}
...
\begin{equation}
\label{eq:3_1_3}
\abs{\Psi}^2 = 4 \abs{\psi_1}^2 \cos{\frac{\Delta \phi}{2}}^2
\end{equation}
...
Квантовое состояние системы полность. определяется вектором состояния $\ket{\psi}$. Векторы $\ket{\psi}$ и $c\ket{\psi} ~(c \neq 0)$ определяют одно и то же состояние
\end{stmt}
...
Векторное просранство состояний наделено скалярным произведением:
$$
...
\noindent Если $\bk{\phi}{\psi} = 0$, то $\phi$ и $\psi$ \textbf{взаимно ортогональны}.
\\
...
\item
	\begin{itemize}
		...
		\item $\bk{\psi}{\psi} = 0 ~\Rightarrow~ \ket{\psi} = 0$
		\item $\norm{\psi} = \sqrt{\bk{\psi}{\psi}}$
	\end{itemize}
\end{enumerate}

Пространство состояний обладает свойством полноты, а значит является \textbf{пространством Гильберта} (обозначается буквой $\mathbb{H}$)

$\ket{\psi}$ можно понимать как столбец: $\left(\vec{\psi} = \begin{array}{c} \psi_1 \\ \psi_2 \end{array}\right)$

$$
\bk{\phi}{\psi} = (\phi_1^*, \phi_2^*) \begin{array}{c} \psi_1 \\ \psi_2 \end{array} = \phi_1^*\psi_1 + \phi_2^* \psi_2
$$
...
\begin{stmt}[Второй постулат квантовой механики\footnote{опять второй?}]
...
\label{eq:3_2_1}
	...
	\end{gathered}
\end{equation}
и соблюдается принцип суперпозиции состояний:\footnote{не уверен, относится ли это к определению}
$$
...
Рассмотри действие $\op{F}$ в гильбертовом пространстве $\mathbb{H}$ (и, соответственно, оператора $\op{F}^\dag$ в $\mathbb{H}^*$):
...
Из \eqref{eq:3_3_3} и \eqref{eq:3_3_4}:
\begin{equation}
\label{eq:3_3_5}
\boxed{
	\bk{\psi_m}{\psi_n} = \delta_{mn}
}
\end{equation}
...
\noindent
$\brcr{\ket{\psi_n}}$ -- условие полноты \footnote{ЧТО?}\\
\\
...
Введём \textbf{оснащённое} гильбертово пространство (т.е. имеющее \footnote{уточнить} обобщённые собственные векторы)
\begin{equation}
\label{eq:3_4_3}
\boxed {
...
Подставляя \eqref{eq:3_4_8} в \eqref{eq:3_2_8} и используя задачи на собственные значения \eqref{eq:3_3_1} и \eqref{3_4_3}, получим:
\begin{equation}
\label{eq:3_4_9}
\boxed {
...
Из \eqref{eq:3_4_8} и \eqref{eq:3_4_9} следует, что $\abs{c_n}^2 = \bk{n}{\psi}$ -- вероятность обнаружить $F$ и состоянии $\ket{\psi}$, ($F = f_n$ \footnote{разобраться})
